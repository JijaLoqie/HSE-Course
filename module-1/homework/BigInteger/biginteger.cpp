...
BigInteger::operator bool() const { return !(_digits.empty() || _digits[0] == 0);}
...
void BigInteger::abs_dif(const BigInteger& right, bool is_lower)
{
        ...
        for (int i = 0; i < min; ++i)
        {
                _digits[i] = (right._digits[i] - _digits[i]) > 0 ?  (right._digits[i] - _digits[i]): (_digits[i] - right._digits[i]);
        }
        if (is_lower)
        {
